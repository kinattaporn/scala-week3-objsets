...
class TweetSetSuite {
  ...
  @Test def `union: with empty set1`: Unit =
    new TestSets {
      set5.foreach(println)
      println("----------")
      set5.union(set1).foreach(println)
  ...
  @Test def `union: with empty set2`: Unit =
    new TestSets {
      ...
      println("----------")
      set1.union(set5).foreach(println)
      assertEquals(4, size(set1.union(set5)))
    }

  @Test def `union: set4c and set2 (already contain)`: Unit =
    new TestSets {
      set4c.foreach(println)
      println("----------")
      set4c.union(set2).foreach(println)
      assertEquals(3, size(set4c.union(set2)))
    }
